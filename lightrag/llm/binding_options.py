...
class BindingOptions:
    ...
    def add_args(cls, parser: ArgumentParser):
        for arg_item in cls.args_env_name_type_value():
            parser.add_argument(
                f"--{arg_item['argname']}",
                type=arg_item["type"],
                default=get_env_value(f"{arg_item['env_name']}", arg_item["default"]),
                help=arg_item["help"],
            )
    ...
    def args_env_name_type_value(cls):
        ...
        for subclass in subclasses:
            args_prefix = f"{subclass._binding_name}"
            env_var_prefix = f"{subclass._binding_name}_".upper()
            class_vars = {
                key: value
                for key, value in BindingOptions._all_class_vars(subclass).items()
    ...
    def generate_dot_env_sample(cls):
        sample_top = (
            "# Autogenerated .env file for LightRAG binding options\n"
            "#\n"
            "# To regenerate a fresh sample .env file, run:\n"
        ...
        for arg_item in cls.args_env_name_type_value():
            if arg_item["help"]:
                sample.append(f"# {arg_item['help']}")
            sample.append(f"# {arg_item['env_name']}={arg_item['default']}\n")
        return "\n".join(sample)
